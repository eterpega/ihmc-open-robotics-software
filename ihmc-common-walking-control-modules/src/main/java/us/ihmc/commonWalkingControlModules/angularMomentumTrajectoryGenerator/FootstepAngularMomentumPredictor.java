...
public class FootstepAngularMomentumPredictor implements AngularMomentumTrajectoryGeneratorInterface
{
   ...
   public FootstepAngularMomentumPredictor(String namePrefix, YoDouble omega0, boolean debug, YoVariableRegistry parentRegistry)
   {
      ...
      for (int i = 0; i < maxNumberOfFootstepsToConsider; i++)
      {
         ...
         AngularMomentumTrajectory transferTrajectory = new AngularMomentumTrajectory(worldFrame, numberOfTransferSegments,
   ...
   private void setSwingFootTrajectoryForFirstTransfer(int footstepIndex)
   {
      ...
      segmentSwingFootPositionTrajectory.setConstant(0.0, currentFirstTransferSegmentDuration, tempFramePoint1);
      TrajectoryMathTools.getDerivative(segmentSwingFootVelocityTrajectory, segmentSwingFootPositionTrajectory);

      if (DEBUG)
         transferSwingFootTrajectories.get(footstepIndex + 1).set(segmentSwingFootPositionTrajectory);
   }
   ...
   private void setSwingFootTrajectoryForSecondTransfer(int footstepIndex, double segmentStartTime)
   {
      ...
      segmentSwingFootPositionTrajectory.setConstant(segmentStartTime, segmentStartTime + currentSecondTransferSegmentDuration, tempFramePoint1);
      TrajectoryMathTools.getDerivative(segmentSwingFootVelocityTrajectory, segmentSwingFootPositionTrajectory);

      if (DEBUG)
         transferSwingFootTrajectories.get(footstepIndex).set(segmentSwingFootPositionTrajectory);
   }

   private void setCoMTrajectoryForSwing(int footstepIndex)
   {
      getCoMEstimationWaypoints(footstepIndex);
      segmentCoMPositionTrajectory.setQuinticWithZeroTerminalAcceleration(-currentSecondTransferSegmentDuration,
                                                                  -currentSecondTransferSegmentDuration + currentFootstepTime, tempFramePoint1,
                                                                  tempFrameVector1, tempFramePoint2, tempFrameVector2);
      segmentCoMPositionTrajectory.setTime(0.0, currentSwingSegmentDuration);
   ...
   {
      getCoMEstimationWaypoints(footstepIndex);
      segmentCoMPositionTrajectory.setQuinticWithZeroTerminalAcceleration(-(currentSecondTransferSegmentDuration + currentSwingSegmentDuration),
                                                                  currentFirstTransferSegmentDuration, tempFramePoint1, tempFrameVector1, tempFramePoint2,
                                                                  tempFrameVector2);
      segmentCoMPositionTrajectory.setTime(0.0, currentFirstTransferSegmentDuration);
      TrajectoryMathTools.getDerivative(segmentCoMVelocityTrajectory, segmentCoMPositionTrajectory);

      if (DEBUG)
         transferCoMTrajectories.get(footstepIndex + 1).set(segmentCoMPositionTrajectory);
   }

   private void setCoMTrajectoryForSecondTransfer(int footstepIndex, double startTime)
   {
      getCoMEstimationWaypoints(footstepIndex);
      segmentCoMPositionTrajectory.setQuinticWithZeroTerminalAcceleration(startTime, startTime + currentFootstepTime, tempFramePoint1, tempFrameVector1,
                                                                  tempFramePoint2, tempFrameVector2);
      segmentCoMPositionTrajectory.setTime(startTime, startTime + currentSecondTransferSegmentDuration);
      TrajectoryMathTools.getDerivative(segmentCoMVelocityTrajectory, segmentCoMPositionTrajectory);

      if (DEBUG)
         transferCoMTrajectories.get(footstepIndex).set(segmentCoMPositionTrajectory);
   }
   ...
   private void setSupportFootTrajectoryForFirstTransfer(int footstepIndex)
   {
      ...
      if (DEBUG)
         transferSupportFootTrajectories.get(footstepIndex + 1).set(segmentSupportFootPositionTrajectory);
   }
   ...
   private void setSupportFootTrajectoryForSecondTransfer(int footstepIndex, double segmentStartTime)
   {
      ...
      if (DEBUG)
         transferSupportFootTrajectories.get(footstepIndex).set(segmentSupportFootPositionTrajectory);
   }
   ...
   private void setSupportFootTrajectory(int footstepIndex, double startTime, double endTime)
   {
      ...
      segmentSupportFootPositionTrajectory.setConstant(startTime, endTime, tempFramePoint1);
   ...
   private void setSwingFootTrajectoryForSwing(int footstepIndex)
   {
      ...
      segmentSwingFootPositionTrajectory.setQuinticWithZeroTerminalAcceleration(0.0, currentSwingSegmentDuration, tempFramePoint1, zeroVector, tempFramePoint2,
                                                                        zeroVector);
