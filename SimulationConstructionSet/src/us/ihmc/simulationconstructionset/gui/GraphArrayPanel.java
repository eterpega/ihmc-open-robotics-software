...
public class GraphArrayPanel extends JPanel implements GraphIndicesHolder, YoGraphRemover, DataBufferChangeListener, Printable, ZoomGraphCommandExecutor {
    ...
    public GraphArrayPanel(SelectedVariableHolder holder, SelectedFilterHolder filterHolder, DataBuffer buffer, JFrame frame, StandardSimulationGUI standardSimulationGUI) {
    ...
    public void setupGraph(String varname) {
        ...
        if (entry != null) {
            EventDispatchThreadHelper.invokeAndWait(new Runnable() {
                @Override
                public void run() {
                    YoGraph g = new YoGraph(getGraphArrayPanel(), getGraphArrayPanel(), selectedVariableHolder, selectedFilterHolder, dataBuffer, dataBuffer, parentFrame);
                    g.addVariable(entry);
                    addGraph(g);
                }
    ...
    public void setupGraph(String[] varnames, GraphConfiguration config) {
        ...
        YoGraph g = new YoGraph(this, this, selectedVariableHolder, selectedFilterHolder, dataBuffer, dataBuffer, parentFrame);
        for (int i = 0; i < varnames.length; i++) {
            DataBufferEntry entry = dataBuffer.getEntry(varnames[i]);
            if (entry != null)
                g.addVariable(entry);
