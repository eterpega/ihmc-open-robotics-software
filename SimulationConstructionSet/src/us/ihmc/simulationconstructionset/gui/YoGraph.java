...
public class YoGraph extends Pane implements EventHandler<Event>
{
   ...
   protected static final int INDIVIDUAL_SCALING = GraphConfiguration.INDIVIDUAL_SCALING, AUTO_SCALING = GraphConfiguration.AUTO_SCALING, MANUAL_SCALING = GraphConfiguration.MANUAL_SCALING;
         ...
         TimeDataHolder timeDataHolder, JFrame jFrame)
   {
      ...
      this.getChildren().addAll(new Canvas(), new Canvas(), new Canvas());
      ...
      colors[0] = Color.rgb(0xa0, 0, 0);
      colors[1] = Color.rgb(0, 0, 0xff);
      colors[2] = Color.rgb(0, 0x80, 0);
      colors[3] = Color.rgb(0, 0, 0);

      colors[4] = Color.rgb(0x80, 0x80, 0x80);
      colors[5] = Color.rgb(0x80, 0, 0x80);
      colors[6] = Color.rgb(0, 0x80, 0x80);
      colors[7] = Color.rgb(0x60, 0x60, 0);
      colors[8] = Color.rgb(0xff, 0x50, 0x50);
      colors[9] = Color.rgb(0x50, 0xff, 0xff);

      baseLineColors[0] = Color.rgb(0x93, 0x70, 0xDB); // Purple
      baseLineColors[1] = Color.rgb(0x3C, 0xB3, 0x71); // Medium sea green
   ...
   /**
    * Calls <code>reCalcMinMax()</code> to find new maximum,
   ...
   /**
    * Calls <code>reCalcMinMax()</code> to find new minimum,
   ...
   /**
    * Calls <code>isEmpty()</code> of this graph's entriesOnThisGraph to check
   ...
   {
      if (this.getWidth() == previousGraphWidth && !override)
      {
         return;
      }
      ...
      {
         int variableWidth;
         int fontSize = (int) ((Canvas) this.getChildren().get(1)).getGraphicsContext2D().getFont().getSize();

         if (showNameSpace)
         {
            variableWidth = fontSize * entry.getFullVariableNameWithNameSpace().length();
         }
         else
         {
            variableWidth = fontSize * entry.getVariableName().length();
         }
   ...
   {
      if (entriesOnThisGraph.size() > 0)
      {
         for (DataEntry entry : entriesOnThisGraph)
         {
            if (entry.minMaxChanged()) {
               return true;
            }
         }
      }
   ...
   private void calcXYData(DataEntry entry, int nPoints, double[] xData, double[] yData, int beginningAt)
   {
      ...
      {
         xData[i] = 0.5 + (int) ((((i + beginningAt) - leftPlotIndex) * this.getWidth()) / (this.graphIndicesHolder.getRightPlotIndex() - leftPlotIndex));
         yData[i] = 0.5 + (int) (height - (((entry.getInverted() ? -data[i] : data[i]) - minVal) / (this.maxFor(entry) - minVal) * height));
      }
   }
   ...
   public void paintTimePlot(int leftPlotIndex, int rightPlotIndex)
   {
      ...
      if (nPoints > 1)
      {
         final double[][] xData = new double[numVars][nPoints], yData = new double[numVars][nPoints];

         for (int i = 0; i < numVars; i++)
         {
            DataEntry entry = entriesOnThisGraph.get(i);

            if (entry.hasDataChanged() || entry.minMaxChanged() || this.shouldRefresh)
            {
               calcXYData(entry, nPoints, xData[i], yData[i], leftPlotIndex);
         ...
         GraphicsContext baselineLayer = ((Canvas) this.getChildren().get(1)).getGraphicsContext2D();
         ...
         for (int i = 0; i < numVars; ++i)
         {
            ...
            Platform.runLater(() ->
            {
               dataLayer.setStroke(colors[x % YoGraph.MAX_NUM_GRAPHS]);
               dataLayer.strokePolyline(xData[x], yData[x], nPoints);
            });

            DataEntry entry = this.entriesOnThisGraph.get(i);
            ...
            if (graphConfiguration.getShowBaseLines())
            {
               ...
               for (int j = 0; j < baseLines.length; j++)
               {
                  ...
                  {
                     double baseY =
                           graphHeight - totalDontPlotBottomPixels - (baseLines[y] - entryMin) / (this.maxFor(entry) - entryMin) * (graphHeight
                                 - totalDontPlotBottomPixels) + 5;

                     baselineLayer.setStroke(baseLineColors[y]);
                     baselineLayer.strokeLine(0, baseY, graphWidth, baseY);
                  });
               }
            }
         }
   ...
   private void paintVerticalIndexLines()
   {
...

      {
         ...
         for (Integer key : keys)
         {
            ...
            {
               linex = ((key * width) / (rightIndex - leftIndex));
   ...
   private void paintVariableNamesAndValues(boolean phasePlot)
   {
      int numVars = entriesOnThisGraph.size();
   ...
   public void handleMouseDragged(javafx.scene.input.MouseEvent evt)
   {
      ...
      if (evt.isPrimaryButtonDown() && (getPlotType() != PHASE_PLOT))
      {
