...
//   private final YoFrameOrientation filteredDesiredCoMOrientation = new YoFrameOrientation(filteredDesiredCoMYaw, filteredDesiredCoMPitch, filteredDesiredCoMRoll, ReferenceFrame.getWorldFrame());
   ...
   private final RecyclingQuadrantDependentList<FramePoint3D> tempSupportPolygonFramePointHolder = new RecyclingQuadrantDependentList<FramePoint3D>(FramePoint3D.class);
...
//   private final YoArtifactPolygon[] tripleSupportArtifactPolygons = new YoArtifactPolygon[6];
   ...
   private final YoDouble comCloseRadius = new YoDouble("comCloseRadius", "Distance check from final desired circle to CoM for transitioning into swing state", registry);
...
//      this.nextSwingLegChooser = new QuadrupedGaitSwingLegChooser(feedForwardReferenceFrames, registry, yoGraphicsListRegistry);
      ...
      for (RobotQuadrant robotQuadrant : RobotQuadrant.values)
      {
         swingTrajectoryGenerators.set(robotQuadrant, new QuadrupedSwingTrajectoryGenerator(robotQuadrant, registry, environment.getGraphicsListRegistry(), dt));
   ...
   */
   ...
   private class QuadrupleSupportState extends State<CrawlGateWalkingState>
   {
      ...
      private void calculateNextCoMTarget(boolean recalculateCurrent)
      {
         ...
         if(recalculateCurrent)
         {
            RobotQuadrant previousSwingLeg = nextSwingLegChooser.chooseNextSwingLeg(fourFootSupportPolygon, currentSwingLeg,
                  lastDesiredVelocity, desiredYawRate.getDoubleValue());
      ...
      public void calculateNextThreeFootSteps(RobotQuadrant firstSwingLeg)
      {
         ...
         // 5.1/7.2 MS 71% START
         if (!oneOffHappened) calculateNextThreeFootStepsOneOffTimer.startMeasurement();
         ...
         if (!oneOffHappened) calculateNextThreeFootStepsOneOffTimer.stopMeasurement();
         // 5.1/7.2 MS 71% STOP
      ...
      private void calculateNextSupportPolygon(QuadrupedSupportPolygon supportPolygon, RobotQuadrant nextSwingLeg, FrameVector3DReadOnly desiredVelocity, double desiredYawRate, QuadrupedSupportPolygon quadrupedSupportPolygonToPack)
      {
         swingDesired.changeFrame(ReferenceFrame.getWorldFrame());
