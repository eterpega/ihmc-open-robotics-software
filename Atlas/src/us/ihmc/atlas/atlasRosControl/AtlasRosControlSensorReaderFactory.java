...
import us.ihmc.atlas.atlasRosControl.dataHolders.YoForceTorqueSensorHandle;
import us.ihmc.atlas.atlasRosControl.dataHolders.YoIMUHandleHolder;
import us.ihmc.atlas.atlasRosControl.dataHolders.YoJointStateHandleHolder;
import us.ihmc.atlas.atlasRosControl.dataHolders.YoMicroStrainIMUHandleHolder;
import us.ihmc.atlas.atlasRosControl.dataHolders.YoPositionJointHandleHolder;
import us.ihmc.atlas.atlasRosControl.dataHolders.YoSwitchableFilterModeIMUHandleHolder;
import us.ihmc.wholeBodyController.diagnostics.JointTorqueOffsetEstimator;

public class AtlasRosControlSensorReaderFactory implements SensorReaderFactory
{

   private StateEstimatorSensorDefinitions stateEstimatorSensorDefinitions;
   private AtlasRosControlSensorReader sensorReader;
   ...
   public AtlasRosControlSensorReaderFactory(TimestampProvider timestampProvider, SensorProcessingConfiguration sensorProcessingConfiguration,
         ...
         HashMap<String, IMUHandle> imuHandles, HashMap<String, ForceTorqueSensorHandle> forceTorqueSensorHandles, AtlasSensorInformation sensorInformation)
   {
   ...
   {
      YoVariableRegistry sensorReaderRegistry = new YoVariableRegistry("AtlasRosControlSensorReader");
      ...
      {
         if (AtlasRosControlController.USE_USB_MICROSTRAIN_IMUS)
         {
            HashMap<String, Integer> imuUSBSerialIds = sensorInformation.getImuUSBSerialIds(); // method does not exist
         ...
         else if (AtlasRosControlController.USE_SWITCHABLE_FILTER_HOLDER_FOR_NON_USB_IMUS)
         {
            ...
            {
               throw new RuntimeException("AtlasRosControlController.USE_SWITCHABLE_FILTER_HOLDER_FOR_NON_USB_IMUS set to true, but cannot find required CF and EF handle names required for this functionality.");
            }
         }
         else
         {
      ...
      sensorReader = new AtlasRosControlSensorReader(stateEstimatorSensorDefinitions, sensorProcessingConfiguration, timestampProvider,
   ...
   @Override
   public AtlasRosControlSensorReader getSensorReader()
   {
      return sensorReader;
   }
