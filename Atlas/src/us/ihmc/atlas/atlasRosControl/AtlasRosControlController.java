...
public class AtlasRosControlController extends IHMCWholeRobotControlJavaBridge
{
   ...
   private final AtlasAffinity atlasAffinity;
   private boolean isGazebo;

   public AtlasRosControlController()
   {
      processEnvironmentVariables();
      atlasAffinity = new AtlasAffinity(!isGazebo);
   }

   private JointTorqueOffsetEstimatorControllerFactory jointTorqueOffsetEstimatorControllerFactory = null;

   private MomentumBasedControllerFactory createDRCControllerFactory(AtlasRobotModel robotModel, PacketCommunicator packetCommunicator,
         DRCRobotSensorInformation sensorInformation)
   {
      ...
      AtlasTorqueOffsetPrinter atlasTorqueOffsetPrinter = new AtlasTorqueOffsetPrinter();
      atlasTorqueOffsetPrinter.setRobotName(robotModel.getSimpleRobotName());
      jointTorqueOffsetEstimatorControllerFactory = new JointTorqueOffsetEstimatorControllerFactory(atlasTorqueOffsetPrinter);
      controllerFactory.addHighLevelBehaviorFactory(jointTorqueOffsetEstimatorControllerFactory);
      PeriodicRealtimeThreadScheduler networkSubscriberScheduler = new PeriodicRealtimeThreadScheduler(AtlasPriorityParameters.POSECOMMUNICATOR_PRIORITY);
   ...
   protected void init()
   {
      ...
      if(isGazebo)
      {
         robotModel = new AtlasRobotModel(AtlasRobotVersion.ATLAS_UNPLUGGED_V5_DUAL_ROBOTIQ,RobotTarget.GAZEBO, true);
      }
      else
      {
      ...
      PeriodicRealtimeThreadSchedulerFactory yoVariableServerScheduler = new PeriodicRealtimeThreadSchedulerFactory(AtlasPriorityParameters.LOGGER_PRIORITY);
      ...
      AtlasRosControlSensorReaderFactory sensorReaderFactory = new AtlasRosControlSensorReaderFactory(timestampProvider, stateEstimatorParameters,
       ...
       */
      AtlasRosControlOutputWriter valkyrieOutputWriter = new AtlasRosControlOutputWriter(robotModel);
      ...
      PeriodicRealtimeThreadScheduler estimatorScheduler = new PeriodicRealtimeThreadScheduler(AtlasPriorityParameters.POSECOMMUNICATOR_PRIORITY);
      ...
      if (isGazebo)
      {
         PrintTools.info(AtlasRosControlController.class, "Running with blocking synchronous execution between estimator and controller");
      ...
      {
         PrintTools.info(AtlasRosControlController.class, "Running with multi-threaded RT threads for estimator and controller");
         MultiThreadedRealTimeRobotController coordinator = new MultiThreadedRealTimeRobotController(estimatorThread);
         if (atlasAffinity.setAffinity())
         {
            coordinator.addController(controllerThread, AtlasPriorityParameters.CONTROLLER_PRIORITY, atlasAffinity.getControlThreadProcessor());
         }
         else
         {
            coordinator.addController(controllerThread, AtlasPriorityParameters.CONTROLLER_PRIORITY, null);
         }
   ...
   protected void doControl(long time, long duration)
   {
      if (firstTick)
      {
         if (atlasAffinity.setAffinity())
         {
            System.out.println("Setting estimator thread affinity to processor " + atlasAffinity.getEstimatorThreadProcessor().getId());
            Affinity.setAffinity(atlasAffinity.getEstimatorThreadProcessor());
         }
         firstTick = false;
      }
