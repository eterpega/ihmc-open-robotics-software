...
import us.ihmc.atlas.atlasRosControl.AtlasTorqueOffsetPrinter;
...
public class AtlasRosControlSliderBoard extends IHMCWholeRobotControlJavaBridge
{
...
//       "rightWristPitch" };
   ...
   private final AtlasRobotModel robotModel = new AtlasRobotModel(AtlasRobotVersion.ATLAS_UNPLUGGED_V5_DUAL_ROBOTIQ,RobotTarget.REAL_ROBOT, true);
   private final FullHumanoidRobotModel fullRobotModel = robotModel.createFullRobotModel();

   private final ArrayList<AtlasSliderBoardJointHolder> jointHolders = new ArrayList<>();
   ...
   // method does not exist in atlas robot model 
   ...
   protected void init()
   {
      ...
      selectedJoint.addVariableChangedListener(new VariableChangedListener()
      {
         ...
         {
            AtlasSliderBoardJointHolder selected = jointHolders.get(selectedJoint.getOrdinal());
   ...
   {
      torqueOffsetMap = AtlasTorqueOffsetPrinter.loadTorqueOffsetsFromFile();
   }
   ...
   protected void doControl(long time, long duration)
   {
      ...
      {
         AtlasSliderBoardJointHolder secondary = jointHolders.get(secondaryJoint.getOrdinal());
      ...
      {
         AtlasSliderBoardJointHolder holder = jointHolders.get(i);
         holder.update();
      }
